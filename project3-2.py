...
class Client_handler(threading.Thread):
	...
	def modify_header(self, strings):
		http_version_idx = strings.find('HTTP/1.1')
		connection_idx = strings.find('keep-alive')
		if not http_version_idx == -1 and not connection_idx == -1:
	...
	def run(self):
		...
		try:
			...
			forward_request = self.modify_header(strings)
			...
			if request_word == 'CONNECT':
				...
				self.nonconnect_to_server(forward_port, forward_host, data)
		...
		while 1:
			if from_client:
				try:
					client_result = self.client_conn.recv(max_bytes)
					sock.send(client_result)
					# sock.send(str.encode(self.modify_header(bytes(client_result))))
				except socket.timeout:
					from_client = not from_client
					continue
				except Exception:
				...
				try:
					server_result = sock.recv(max_bytes)
					self.client_conn.send(server_result)
					# self.client_conn.send(str.encode(self.modify_header(bytes(server_result))))
				except socket.timeout:
					from_client = not from_client
					continue
				except Exception:
